...
class MeshDrawer {
	...
	draw(trans) {
		...
		updateLightPos();
    	this.lightPos = [lightX, lightY, 1.0];  // Update light position in 3D space (using Z=1.0 for example)
	...
	setTexture(img) {
		...
		if (isPowerOf2(img.width) && isPowerOf2(img.height)) {
			 ...
			 */
			console.warn("Task 1: Non power of 2 texture detected. Implementing NPOT support.");
    
			// Set texture wrapping to clamp-to-edge for NPOT textures
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);

			// Set texture filtering to linear for NPOT textures
...
const meshFS = `
			...
			void main()
			{
				if (enableLighting) {
					...
					// Compute the diffuse intensity (dot product of light direction and normal)
